...
public static class GraphicsRuntimeSettings
{
    ...
    // ĐÃ XÓA việc chỉnh fieldOfView — camera giữ nguyên FOV mặc định
...
public class GraphicsMenu : MenuBase
{
    ...
    [SerializeField] private int[] frameRates = { 30, 60, 144, -1 }; // -1 represents Unlimited
    ...
    // Cache text để chống AutoTranslateUI ghi đè
    private string cachedFrameRateText = "";
    ...
    // LateUpdate chống AutoTranslateUI ghi đè text
    private void LateUpdate()
    {
        EnforceFrameRateText();
    }

    private void EnforceFrameRateText()
    {
        if (frameRateSelector == null || frameRateSelector.valueText == null)
            return;

        if (frameRateSelector.valueText.text != cachedFrameRateText)
        {
            frameRateSelector.valueText.text = cachedFrameRateText;
        }
    }
    ...
    {
        int savedFrameRate = PlayerPrefs.GetInt("Graphics.FrameRate", 60);
        frameRateIndex = FindIndex(frameRates, savedFrameRate, 1);

        Debug.Log($"[GraphicsMenu] LoadSettings: Saved FrameRate = {savedFrameRate}, Index = {frameRateIndex}, Array Value = {frameRates[frameRateIndex]}");
    ...
    private void RefreshSelectors()
    {
        ...
        cachedFrameRateText = frameRate < 0 ? "Unlimited" : frameRate.ToString();

        frameRateSelector.SetText(cachedFrameRateText);
        ...
        Debug.Log($"[GraphicsMenu] RefreshSelectors: FrameRate Text = {cachedFrameRateText}");
    }
    ...
    private int FindIndex(int[] values, int target, int fallback)
    {
        ...
        if (index < 0)
        {
            Debug.LogWarning($"[GraphicsMenu] FindIndex: Target {target} not found in array. Using fallback {fallback}");
        }
