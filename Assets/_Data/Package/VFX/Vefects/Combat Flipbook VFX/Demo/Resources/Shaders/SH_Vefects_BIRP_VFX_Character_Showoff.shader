...
Shader "Vefects/SH_Vefects_BIRP_VFX_Character_Showoff"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			float _EdgesBias;
			float4 _WorldNoiseTiling;
			float4 _WorldNoiseSpeed;
			float _NoiseDepth;
			float _BaseColorEdgesLerp;
			CBUFFER_END

			TEXTURE2D(_NoiseEdgesTexture);
			SAMPLER(sampler_NoiseEdgesTexture);
			...
			half4 frag( Varyings input ) : SV_Target
			{
				...
				float edgeMask = saturate( pow( fresnel, max( _EdgesPower, 0.0001 ) ) * _EdgesMultiply + _EdgesBias );
				float2 noiseUV = ( ( input.positionWS.xy + ( edgeMask * _NoiseDepth ) ) * _WorldNoiseTiling.xy ) + ( _Time.y * _WorldNoiseSpeed.xy );
				float noiseValue = SAMPLE_TEXTURE2D( _NoiseEdgesTexture, sampler_NoiseEdgesTexture, noiseUV ).r;
				float edgeBlend = edgeMask * noiseValue;
				float4 edgeColor = lerp( _BaseColor, _EdgesColor, edgeBlend );
				float4 baseColor = lerp( _BaseColor, edgeColor, _BaseColorEdgesLerp );

				Light mainLight = GetMainLight();
				float ndotl = saturate( dot( normalWS, mainLight.direction ) );
				float specPower = lerp( 8.0, 64.0, saturate( _Smoothness ) );
				float3 halfDir = SafeNormalize( mainLight.direction + viewDirWS );
				float specTerm = pow( saturate( dot( normalWS, halfDir ) ), specPower ) * _Specular;
				float3 diffuse = baseColor.rgb * ( 0.18 + ( ndotl * mainLight.shadowAttenuation * mainLight.distanceAttenuation ) );
				float3 specular = specTerm * mainLight.color;
				float3 emission = ( _EdgesColor.rgb * edgeBlend ) * _EmissionIntensity;

				return half4( diffuse + specular + emission, 1.0 );
			}
			ENDHLSL
		}
	}
