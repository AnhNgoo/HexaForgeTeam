...
Shader "Vefects/SH_Vefects_BIRP_Unlit_Combat_Flipbook_Advanced_01"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				float2 panner140 = ( _Time.y * _MaskUVPan + ( input.uv.xy * _MaskUVScale ) );
				float2 flipbookFrames = float2( _FlipbookX , _FlipbookY );
				float2 panner127 = ( _Time.y * _DistortionUVPan + ( ( input.uv.xy * _DistortionUVScale ) * flipbookFrames ) );
				float randomOffset = input.uv2.x;
				float2 distortionSample = ( ( SAMPLE_TEXTURE2D( _DistortionTexture, sampler_DistortionTexture, ( panner127 + randomOffset ) ).rg + -0.5 ) * 2.0 );
				float2 distortion = lerp( float2( 0, 0 ) , distortionSample , _DistortionLerp );
				float2 prePixelateUVs = ( panner140 + distortion );
				float pixelWidth = 1.0f / ( _PixelsX * _PixelsMultiplier );
				float pixelHeight = 1.0f / ( _PixelsY * _PixelsMultiplier );
				half2 pixelatedUV = half2( (int)( prePixelateUVs.x / pixelWidth ) * pixelWidth, (int)( prePixelateUVs.y / pixelHeight ) * pixelHeight );
				#ifdef _PIXELATE_ON
					float2 finalUVs = pixelatedUV;
				#else
					float2 finalUVs = prePixelateUVs;
				#endif
				float4 maskSample = SAMPLE_TEXTURE2D( _MaskTexture, sampler_MaskTexture, finalUVs );
				float4 lerpResult97 = lerp( _Outline , _B , maskSample.b );
				float4 lerpResult112 = lerp( lerpResult97 , _G , maskSample.g );
				float4 lerpResult111 = lerp( lerpResult112 , _R , maskSample.r );
				float4 baseColor = lerp( ( input.color * lerpResult111 ) , input.color , _FlatColor );
				float4 color183 = ( baseColor * _Emissive );

				float4 alphaSample = SAMPLE_TEXTURE2D( _AlphaTexture, sampler_AlphaTexture, finalUVs );
				float lerpResult186 = lerp( alphaSample.g , alphaSample.r , saturate( _AlphaGlow ) );
				float mainTexAlpha = saturate( lerpResult186 );
				float opacityVtcW = input.uv.z;
				float temp_output_208_0 = saturate( opacityVtcW );
				float2 panner216 = ( _Time.y * _ErosionNoiseUVPan + ( ( input.uv.xy * flipbookFrames ) * _ErosionNoiseUVScaleOverall ) );
				float smoothstepResult200 = smoothstep( temp_output_208_0 , ( temp_output_208_0 + _ErosionSmoothness ) , SAMPLE_TEXTURE2D( _ErosionNoise, sampler_ErosionNoise, ( ( panner216 + randomOffset ) + distortion ) ).g );
				float lerpResult196 = lerp( mainTexAlpha , saturate( ( mainTexAlpha * saturate( smoothstepResult200 ) ) ) , _ErosionIntensity );
				float mainTexVcAlpha = input.color.a;
				float depthFade = 1.0;
				if ( _DepthFade > 0.0001 )
				{
					float2 screenUV = GetNormalizedScreenSpaceUV( input.positionCS );
					float sceneDepth = SampleSceneDepth( screenUV );
					#if UNITY_REVERSED_Z
						bool hasSceneDepth = ( sceneDepth < 0.9999 );
					#else
						bool hasSceneDepth = ( sceneDepth > 0.0001 );
					#endif
					if ( hasSceneDepth )
					{
						float sceneEyeDepth = LinearEyeDepth( sceneDepth, _ZBufferParams );
						float fragEyeDepth = LinearEyeDepth( input.positionCS.z / input.positionCS.w, _ZBufferParams );
						depthFade = saturate( ( sceneEyeDepth - fragEyeDepth ) / _DepthFade );
					}
				}
				float opacityRegister = saturate( saturate( lerpResult196 ) * mainTexVcAlpha );
				return half4( color183.rgb, opacityRegister );
			}
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutput o )
		{
			...
			float distanceDepth211 = 1.0;
			if ( _DepthFade > 0.0001 )
			{
				float rawSceneDepth = SAMPLE_DEPTH_TEXTURE( _CameraDepthTexture, ase_screenPosNorm.xy );
				#if defined(UNITY_REVERSED_Z)
					bool hasSceneDepth = ( rawSceneDepth < 0.9999 );
				#else
					bool hasSceneDepth = ( rawSceneDepth > 0.0001 );
				#endif
				if ( hasSceneDepth )
				{
					float screenDepth211 = LinearEyeDepth( rawSceneDepth );
					distanceDepth211 = saturate( ( screenDepth211 - LinearEyeDepth( ase_screenPosNorm.z ) ) / ( _DepthFade ) );
				}
			}
			float OpacityRegister179 = saturate( saturate( lerpResult196 ) * mainTex_VC_alha52 );
