...
public class EarthRages : CharacterSkillBase
{
    ...
    protected override async void ExecuteSkill()
    {
        ...
        if (isExecuting || kael.IsGiantForm)
            return;
        ...
        try
        {
            ...
            // Đợi animation mở đầu đủ lâu rồi mới chuyển sang dạng khổng lồ.
           ...
           kael.bottomEffectPoint.transform);
            ...
            // Đợi animation biến hình hoàn tất rồi cho phép Kael chiến đấu.
            ...
            // Đợi animation kết thúc rồi mới trả Kael về dạng thường.
            await UniTask.WaitUntil(() => character.CharacterAnimation.GetAnimationTime("Skill_2_1") > 0.15f &&
                      !character.CharacterAnimation.Animator.IsInTransition(0));
        ...
        {
            // Bất kể luồng skill kết thúc thế nào, không để Kael mắc kẹt ở dạng khổng lồ.
            if (kael.IsGiantForm)
                kael.NormalForm();
