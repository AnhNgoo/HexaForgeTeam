...
namespace DuskBlade.Tests
{
    public class MapCollisionSystemTests : RuntimeSystemTestBase
    {
        ...
        [UnityTest, Category("MapCollision"), Category("Tu dong"), Description("MAP-001: Kiem tra load duoc scene Tutorial.")]
        public IEnumerator MAP_001_LoadGameplaySceneDuoc() { return RunUnity("MAP-001", "Load scene Tutorial duoc", "Load duoc scene Tutorial that trong project.", "High", c => Load(c)); }

        [UnityTest, Category("MapCollision"), Category("Tu dong"), Description("MAP-002: Ghi nhan collider trong scene Tutorial.")]
        public IEnumerator MAP_002_SceneCoCollider() { return RunUnity("MAP-002", "Scene Tutorial collider", "Ghi nhan collider trong scene Tutorial neu co.", "Low", c => SceneCollider(c)); }

        [UnityTest, Category("MapCollision"), Category("Tu dong"), Description("MAP-003: Ghi nhan collider non-trigger trong scene Tutorial.")]
        public IEnumerator MAP_003_ColliderNenKhongTrigger() { return RunUnity("MAP-003", "Collider non-trigger Tutorial", "Ghi nhan collider khong trigger trong scene Tutorial neu co.", "Low", c => GroundCollider(c)); }
        ...
        [UnityTest, Category("MapCollision"), Category("Tu dong"), Description("MAP-008: Ghi nhan bounds collider trong scene Tutorial.")]
        public IEnumerator MAP_008_ColliderMapBoundsHopLe() { return RunUnity("MAP-008", "Collider Tutorial bounds", "Ghi nhan bounds collider trong scene Tutorial neu co.", "Low", c => Bounds(c)); }
        ...
        [UnityTest, Category("MapCollision"), Category("Tu dong"), Description("MAP-010: Kiem tra scene Tutorial chay 60 frame.")]
        public IEnumerator MAP_010_MapChay60FrameKhongLoi() { return RunUnity("MAP-010", "Tutorial scene chay 60 frame", "Scene Tutorial that chay 60 frame trong PlayMode.", "High", c => Sixty(c)); }

        private IEnumerator Load(Ctx c) { yield return LoadGameplayScene(c); yield return null; c.Actual += "Scene Tutorial load thanh cong."; }
        private IEnumerator SceneCollider(Ctx c) { yield return LoadGameplayScene(c); yield return null; Collider[] colliders = UnityEngine.Object.FindObjectsOfType<Collider>(true); int enabled = 0; foreach (Collider col in colliders) if (col.enabled) enabled++; c.Actual += $"Collider tong={colliders.Length}, enabled={enabled}."; }
        private IEnumerator GroundCollider(Ctx c) { yield return LoadGameplayScene(c); yield return null; Collider[] colliders = UnityEngine.Object.FindObjectsOfType<Collider>(true); int nonTrigger = 0; foreach (Collider col in colliders) if (col.enabled && !col.isTrigger) nonTrigger++; c.Actual += $"Collider khong trigger={nonTrigger}."; }
