...
namespace DuskBlade.Tests
{
    public class CameraSystemTests : RuntimeSystemTestBase
    {
        ...
        [UnityTest, Category("Camera"), Category("Tu dong"), Description("CAM-001: Kiem tra scene Run Game load duoc de kiem tra camera.")]
        public IEnumerator CAM_001_TimDuocCameraThat() { return RunUnity("CAM-001", "Scene Run Game load duoc", "Load duoc scene Run Game de kiem tra camera.", "High", c => LoadRunScene(c)); }
        ...
        private IEnumerator LoadRunScene(Ctx c) { yield return LoadSceneByPath(TestSceneConfig.RunScenePath, c); yield return null; c.Actual += "Scene Run Game load thanh cong."; }
        private IEnumerator SceneCamera(Ctx c) { yield return LoadSceneByPath(TestSceneConfig.RunScenePath, c); yield return null; Camera[] cameras = UnityEngine.Object.FindObjectsOfType<Camera>(true); c.Actual += $"Camera trong scene={cameras.Length}, Camera.main={(Camera.main != null)}."; }
        private IEnumerator CameraComponent(Ctx c) { yield return LoadSceneByPath(TestSceneConfig.RunScenePath, c); yield return null; int unity = UnityEngine.Object.FindObjectsOfType<Camera>(true).Length; int cine = CountSceneComponents("CinemachineVirtualCamera"); c.Actual += $"Unity Camera={unity}, CinemachineVirtualCamera={cine}."; }
        private IEnumerator MainCamera(Ctx c) { yield return LoadSceneByPath(TestSceneConfig.RunScenePath, c); yield return null; Camera camera = Camera.main ?? UnityEngine.Object.FindObjectOfType<Camera>(); c.Actual += $"Camera ton tai={(camera != null)}, tag={(camera != null ? camera.tag : "null")}, Camera.main={(Camera.main != null)}."; }
        ...
        private IEnumerator Settings(Ctx c) { yield return LoadSceneByPath(TestSceneConfig.RunScenePath, c); yield return null; Camera camera = Camera.main ?? UnityEngine.Object.FindObjectOfType<Camera>(); if (camera == null) { c.Actual += "Scene Run Game khong co Camera runtime bat buoc."; yield break; } c.Actual += $"FOV={N(camera.fieldOfView)}, near={N(camera.nearClipPlane)}, far={N(camera.farClipPlane)}."; Assert.Greater(camera.fieldOfView, 1f); Assert.Greater(camera.farClipPlane, camera.nearClipPlane); }
        private IEnumerator Distance(Ctx c) { yield return LoadSceneByPath(TestSceneConfig.RunScenePath, c); yield return null; Camera camera = Camera.main ?? UnityEngine.Object.FindObjectOfType<Camera>(); GameObject player = GameObject.FindWithTag("Player"); if (camera == null || player == null) { c.Actual += $"Camera={(camera != null)}, Player={(player != null)}; scene Run Game khong bat buoc co ca hai object."; yield break; } float distance = Vector3.Distance(camera.transform.position, player.transform.position); c.Actual += $"Camera={camera.name}, Player={player.name}, khoang cach={N(distance)}."; Assert.Greater(distance, 0.5f); Assert.Less(distance, 120f); }
        private IEnumerator Active(Ctx c) { yield return LoadSceneByPath(TestSceneConfig.RunScenePath, c); yield return null; Camera camera = Camera.main ?? UnityEngine.Object.FindObjectOfType<Camera>(); c.Actual += $"Camera ton tai={(camera != null)}, enabled={(camera != null && camera.enabled)}."; if (camera != null) Assert.IsTrue(camera.enabled); }
